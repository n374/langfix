...
> 「覆盖测试」：round1 与 round2 的 Scenario 均已用真实测试路径（`Tests/LangFixTests/*`）标注；**round3 新增的对抗验收 Scenario 已替换为真实测试路径，测试驱动真实测量路径（`ReviewMeasurementView` / `NSHostingController.sizeThatFits` / `ReviewWindowController.measureAndApplyForTesting`）、逐帧断言、含必测失败路径——不得复用「直接喂入正确自然高度」的算术级单测充数**（用户 `e698b62a` 明确「修到对抗评审通过为止」）。
...
**覆盖测试**: `Tests/LangFixTests/ReviewWindowStyleTests.swift::testLoadingMeasurementAndAppliedHeightDoNotLeaveLargeFooterWhitespace`
...
**覆盖测试**: `Tests/LangFixTests/ReviewWindowSizingTests.swift::testHeightGrowsThenCapsAtMaxH`、`Tests/LangFixTests/ReviewWindowStyleTests.swift::testLongStreamingContentCapsAtMaxHeightAndMarksOverflow`
...
**覆盖测试**: `Tests/LangFixTests/ReviewWindowStyleTests.swift::testShortLoadingMeasurementDoesNotGreedyFillTallContainer`、`Tests/LangFixTests/ReviewWindowStyleTests.swift::testShortStreamingMeasurementDoesNotFillMaxHeight`
...
**覆盖测试**: `Tests/LangFixTests/ReviewWindowStyleTests.swift::testStreamingPeakFallsBackThroughRealMeasurementAndApplyPath`、`Tests/LangFixTests/ReviewWindowSizingTests.swift::testHeightCanFallBackWhenStreaming`
...
**覆盖测试**: `Tests/LangFixTests/ReviewWindowStyleTests.swift::testMaxHeightFrameDoesNotPolluteNextShortMeasurementAndApply`、`Tests/LangFixTests/ReviewWindowStyleTests.swift::testStreamingPeakFallsBackThroughRealMeasurementAndApplyPath`
...
**覆盖测试**: `Tests/LangFixTests/ReviewWindowStyleTests.swift::testInitialFrameIsUpperRightBiasedAndInsideVisibleFrame`
