...
**覆盖测试**: TBD(unit/ui: 模拟无语言键升级 → 断言自动配置且不强制 onboarding)
...
**覆盖测试**: TBD(ui: {中,英} 两态 → 断言 UI 文案随用户语言)
...
**覆盖测试**: TBD(unit: 旧 _zh payload → 断言解析成功且内容完整)
...
**覆盖测试**: TBD(unit: 缺字段 payload → 断言 fail loud 非静默)
...
**覆盖测试**: TBD(unit: 目标=英 + 夹中文 → 断言夹带片段转写为英文、非保留；original 为本地输入)
...
**覆盖测试**: TBD(unit: 目标=中 + 夹英文 → 断言夹带片段转写为中文)
...
**覆盖测试**: TBD(unit: 混排高比例 → 断言 strict retry 被触发、流程与非混排一致)
...
**覆盖测试**: TBD(unit: 用户语言=英 → 断言解释/总评/直译为英文)

#### Scenario: 用户语言=中文回归不变
- **GIVEN** 用户语言=中文
- **THEN** 解释/总评/直译为中文（与现状一致）

**覆盖测试**: TBD(unit: 用户语言=中 → 断言中文解释，回归)
...
**覆盖测试**: TBD(unit: 通用语法追问 → 断言得到解答而非拒答)

#### Scenario: 放宽后仍不吐可替代主结果的全文
- **GIVEN** 追问放宽已生效
- **WHEN** 用户贴一段新文本要求「给我一版全文 / 整段翻译改写」
- **THEN** 现有输出护栏仍拦截可作整段替换结果的全文；可给解释或**短例句**，但不产出替代 corrected；对新文本纠错/翻译应走一次新的 review 链路，`corrected`/`issues` 不变

**覆盖测试**: TBD(unit: 放宽后贴新文本要全文 → 断言输出护栏拦截、主结果不变、引导走新 review)
...
**覆盖测试**: TBD(unit: 放宽后注入样例 → 断言不越权、主结果不变)
