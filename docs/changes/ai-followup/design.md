...
| 2026-07-15 | 用户 review PR#4 四条调整（开发官）：①`.result` 态底部输入框+按钮移出滚动区 pin 到窗口底部（ResultView 自管中部 ScrollView + 固定 footer），仅结果/追问内容滚动；②③review prompt 加硬约束 8（original/corrected 逐字保真、换行/空白/标点不规范化）+ ReviewEngine 换行结构闭环：`collapsedNewlines`（两侧 `normalizedLineEndings` 后计数）检测多行 corrected 被合并 → 触发 strict 重试、仍破坏则 `pickBetter`（保留换行优先）+`overEdited` 警示，绝不静默把结构违规当干净；输入边界 `handleSelection` 先归一化换行（CRLF/CR/U+2028/U+2029→LF）再 trim；④结果区 `.textSelection(.enabled)` 全覆盖、IssueCard 引用改显式 quote 按钮避免抢选择手势。+7 单测（175 全绿）。Codex 对抗式评审 3 轮收敛（1 高·换行无客户端闭环 → CRLF 异体换行边界 → 通过） | 开发官 |
| 2026-07-16 | 换行检测对齐用户验收标准（开发官）：MR 门禁先后发现「补偿换行」「补空行」可绕过总数/内部换行计数；用户 2026-07-16 明确验收——**内容换行必须保留、多个空行合并成单行可接受**。据此 `collapsedNewlines` 改用**单一指标 `nonEmptyLineCount`**（仅非空内容行数下降才判「内容换行丢失」→ strict/`overEdited`），移除 `internalNewlineCount`（它把空行收缩误判为破坏、过严）。此指标对补空行/补偿换行**结构性免疫**（空行不计）。已知残留：行数不变的断行位置重排需内容对齐检测（会误判合法最小改动），用户已知悉并接受本方案。+调整单测（179 全绿）。Codex 复核通过 | 开发官 |
| 2026-07-16 | 修复「追问增长窗口高度未封顶超过屏幕」（开发官）：根因是窗口尺寸自适应只订阅 `state.objectWillChange`，而追问内容挂在嵌套 `state.followUp`（独立 ObservableObject），其变化不 fire state → 不触发 remeasure/`applyResize`(clamp maxH)/`isOverflowing` 翻转 → 窗口无上限增长。修复：`ReviewWindowController` 新增 `followUpChangeCancellable`，注入后订阅 `followUp.objectWillChange → refreshMeasurement`，`close()` 清理。每次追问增长走一遍封顶+溢出切换（中部滚动、底栏固定，不回退 adj#1）。+回归用例（走生产 runloop 路径，已验证「去掉修复即失败」，180 全绿）。Codex 复核通过 | 开发官 |
