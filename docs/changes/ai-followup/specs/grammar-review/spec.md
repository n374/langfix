...
**覆盖测试**: TBD(unit: 含 3 条 issues 的定稿 ReviewResult → 断言展示序号 1..N 与上下文编号同源且稳定)
...
**覆盖测试**: TBD(unit: 预览态 → 断言无可引用序号；strict 覆盖前后 → 断言定稿序号不漂移)
...
**覆盖测试**: TBD(unit: 空 issues → 断言无序号生成)
...
**覆盖测试**: TBD(unit: 带序号修正清单 + 引用「修正 2」→ 断言上下文序号绑定正确、追问前后 corrected/issues 不变)
...
**覆盖测试**: TBD(unit: 两轮追问 → 断言第二轮上下文含第一轮问答且 corrected/issues 不变)
...
**覆盖测试**: TBD(unit: 改写型追问 → 断言主结果不变且未生成替代 corrected)
...
**覆盖测试**: TBD(manual: 跑题追问 → 人工核对不偏离产品定位)
...
**覆盖测试**: TBD(unit: 引用越界/不存在序号 → 断言不发含糊请求并给提示)
...
**覆盖测试**: TBD(unit: 追问注入样例 → 断言不越权、主结果不变)
...
**覆盖测试**: TBD(unit: 原文含注入 + 追问 → 断言答疑不被劫持)
...
**覆盖测试**: TBD(unit: 在途取消 → 断言请求中止、历史不含该轮、主结果不变)
...
**覆盖测试**: TBD(unit: 关窗/新 generation 后旧响应到达 → 断言被丢弃且无副作用)
...
**覆盖测试**: TBD(unit: 构造超预算上下文 + 引用某修正 → 断言被引用修正保留或明确报错，无静默错位)
...
**覆盖测试**: TBD(unit: 超限失败 → 断言历史未被写入、可重试复用绑定)
...
**覆盖测试**: TBD(unit: 追问返回 401 → 断言错误提示、主结果不变、历史未写入)
...
**覆盖测试**: TBD(unit: 分别 mock 超时/429/5xx → 断言可重试、复用绑定、主结果不变)
...
**覆盖测试**: TBD(ui: 给定 mock ReviewResult → 断言 corrected/diff/issue 三区渲染且 issue 带序号)
