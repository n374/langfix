...
**覆盖测试**: TBD(unit: 保存配置 → 断言 Keychain 有 key、UserDefaults 无 key)
...
**覆盖测试**: TBD(unit: editRatio>阈值 → 断言 strict 模式被调用一次)
...
**覆盖测试**: TBD(unit: 两轮均超阈值 → 断言 overEdited=true)
...
**覆盖测试**: TBD(unit: mock 400 链 → 断言三级降级顺序)
...
**覆盖测试**: TBD(unit: 401 → 断言错误类型=auth；ui: 显示设置入口)
...
**覆盖测试**: TBD(unit: 保存 key → 断言 UserDefaults 无 key、Keychain 有)
...
**覆盖测试**: TBD(unit: 构造 original 回显不符 → 断言以本地输入为基准)
